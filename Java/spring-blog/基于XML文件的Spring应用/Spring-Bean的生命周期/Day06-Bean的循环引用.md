...
2.  *userService*改造
    ...
    2.  通过对应objectFactory.getObject()得到返回值*userService*(**未被别人引用**)
    3.  将*userService*(**未被别人引用**)**存入三级缓存**
...
4.  去创建userDao
    ...
    2.  userDao改造
        ...
        2.  通过对应objectFactory.getObject()得到返回值userDao(**未被别人引用**)
        3.  将userDao(**未被别人引用**)**存入三级缓存**
...
5.  *userService*注入userDao(在一级缓存中找到目标)
