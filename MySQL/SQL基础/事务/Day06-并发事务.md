...
- 脏读
  ...
  事务 A 写入数据, 还没有提交, 事务 B 就读到了, 事务 A 发生回滚, 事务B使用的是脏数据

- 不可重复度

  事务 A 查询到一条数据, 事务B修改了这条数据并提交, 而后事务 A 再次查询发现, 两条数据不一致

- 幻读

  事务 A 按照查出一批数据, 而后事务B插入了另一批数据并提交, 此时事务 A 再去读, 发现两次读取, 发现数据不一致
